...
export default function Vyroba({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  function handleVyrobaProjektTouchMove(e: React.TouchEvent) {
    ...
    if (overlay) { overlay.style.transition = "none"; overlay.style.opacity = String(1 - Math.min(deltaY / 300, 1)); }
  }
  function handleVyrobaProjektTouchEnd() {
    ...
    if (finalY > 80) {
      el.style.transition = "transform 0.2s ease"; el.style.transform = `translateY(${el.offsetHeight}px)`;
  ...
  async function recalculateScheduledHours(projectId: string) {
    try {
      ...
      toast.error("Chyba při přepočtu");
    }
  }
  ...
  return (
    ...
    >
      {/* ═══ MOBILE HEADER ═══ */}
      {!embedded && isMobile && (
        <MobileHeader onDataLog={() => setDataLogOpen(true)} showDataLog={isAdmin || role === "pm" || isOwner} isDataLogOpen={dataLogOpen} onCloseDataLog={() => setDataLogOpen(false)} />
      )}
              ...
              {PAGER_OFFSETS.map(offset => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      Projekty v T{slideWeekNum} ({slideProjects.filter((p) => !p.isPaused).length})
                    </div>
          ...
          <Sheet open={mobileVyrobaProjektOpen} onOpenChange={setMobileVyrobaProjektOpen}>
            ...
            >
              <div className="flex flex-col h-full">
                ...
                {todayDayIndex >= 0 && (
                  <div className="shrink-0 px-4 pt-3 pb-5 mb-1 border-t border-border bg-background">
                    <button
            ...
            return (
              <Sheet open={mobileDaylogOpen} onOpenChange={setMobileDaylogOpen}>
                ...
                >
                  ...
                  <div className="shrink-0 px-4 pt-3 pb-5 mb-1 border-t border-border bg-background space-y-2">
