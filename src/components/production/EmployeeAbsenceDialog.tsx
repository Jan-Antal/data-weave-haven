...
export function EmployeeAbsenceDialog({ open, onOpenChange, employeeId, employeeName }: Props) {
  ...
  const cleanup = useCleanupNonWorkdayAbsences();
  ...
  const handleCleanup = async () => {
    if (!confirm("Smazat všechny manuální absence z víkendů a státních svátků (napříč všemi zaměstnanci)?")) return;
    try {
      const res = await cleanup.mutateAsync();
      toast({ title: `Vyčištěno: ${res.removed} záznamů` });
    } catch (e: any) {
      toast({ title: "Chyba", description: e.message, variant: "destructive" });
    }
  };
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-2 border-t pt-4">
          ...
          <div className="text-[11px] text-muted-foreground italic">
            Záznamy se zapíší pouze na pracovní dny — víkendy a státní svátky budou přeskočeny.
          </div>
        </div>
        ...
        <DialogFooter className="flex sm:justify-between gap-2">
          <Button
            ...
            className="text-xs"
            title="Smaže existující manuální absence z víkendů a svátků"
          >
            <Sparkles className="h-3 w-3 mr-1" />
            {cleanup.isPending ? "Čistím..." : "Vyčistit víkendy/svátky"}
          </Button>
