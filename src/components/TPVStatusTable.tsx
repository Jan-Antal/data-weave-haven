...
const TPVListIcon = memo(function TPVListIcon({ projectId, itemCount, onClick }: { projectId: string; itemCount: number; onClick: () => void }) {
  ...
  return (
    <button
      ...
      style={{ color: hasItems ? "#223937" : "#99a5a3" }}
...
const TPVProjectRow = memo(function TPVProjectRow({
  ...
  return (
    <TableRow className={cn("hover:bg-muted/50 transition-colors h-9", isSummary && "bg-muted/40")} style={tpvHighlight.bg ? { backgroundColor: tpvHighlight.bg } : {}} data-project-id={p.project_id}>
      ...
      {renderKeys.map((key) => renderColumnCell({ colKey: key, project: displayProject, save, canEdit: canEdit && (stageCount <= 1 || key === "architekt" || key === "klient"), statusLabels, customColumns, saveCustomField: (rowId, colKey, val, old) => saveCustomField(rowId, colKey, val, old), {renderKeys.map((key) => renderColumnCell({ colKey: key, project: displayProject, save, canEdit: canEdit && (stageCount <= 1 || key === "architekt" || key === "klient" || key === "pm_poznamka" || key === "tpv_poznamka"), statusLabels, customColumns, saveCustomField: (rowId, colKey, val, old) => saveCustomField(rowId, colKey, val, old), isFieldReadOnly: stageCount > 1 ? (field) => field !== "architekt" && field !== "klient" && field !== "pm_poznamka" && field !== "tpv_poznamka" : isFieldReadOnly, isSummaryRow: isSummary }))}, isSummaryRow: isSummary }))}
    </TableRow>
  );
});
