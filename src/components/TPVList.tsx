              ...
              {isLoading ? (
                ...
                <TableRow key={item.id} className={`hover:bg-muted/50 transition-colors h-9 ${selected.has(item.id) ? "bg-primary/5" : ""}`}>
                  ...
                  {renderKeys.map(key => {
                    ...
                    if (key === "cena") return <TableCell key={key} style={cellStyle} className="text-right"><InlineEditableCell value={String(item.cena ?? "")} type="number" onSave={(v) => saveField(item.id, "cena", v, String(item.cena ?? ""))} readOnly={!canManageTPV} suffix={` ${currency}`} /></TableCell>;
                    // Custom columns
