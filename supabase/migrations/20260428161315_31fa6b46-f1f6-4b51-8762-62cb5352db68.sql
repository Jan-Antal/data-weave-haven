...
AS $function$
  ...
  -- Spillover: ONLY from immediately previous week (matches UI prevWeekInfo.weekKey)
  ...
  -- Off-plan projects: have ALVENO hours this week but no schedule, no spillover, no daily log.
  -- Mirror UI "Mimo Plán výroby" cards. Synthesize a single placeholder row per project.
  offplan_projects AS (
    SELECT
      wap.project_id,
      NULL::uuid AS stage_id,
