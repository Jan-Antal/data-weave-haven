...
AS $function$
  ...
  -- Parse logs: detect SG:<uuid> format vs full 5-segment format
  raw_logs AS (
    SELECT
  ...
  logs_resolved AS (
    -- Path A: full format (5 segments) — extract directly
    SELECT
      rl.raw_bundle_id AS bundle_id,
      rl.phase, rl.percent, rl.note_text, rl.logged_at, rl.day_index,
      rl.parts[1] AS p_project_id,
      NULLIF(rl.parts[4], '') AS p_bundle_label,
      NULLIF(NULLIF(rl.parts[5], ''), 'full') AS p_bundle_split_part,
      (rl.parts[2]::date + rl.day_index) AS p_log_day_date
    FROM raw_logs rl
    WHERE array_length(rl.parts, 1) >= 5
      AND rl.parts[3] NOT LIKE 'SG:%'
    UNION ALL
    -- Path B: SG:<uuid> format — resolve label/split via production_schedule
    SELECT
      rl.raw_bundle_id AS bundle_id,
      rl.phase, rl.percent, rl.note_text, rl.logged_at, rl.day_index,
      rl.parts[1] AS p_project_id,
      sg.bundle_label AS p_bundle_label,
      CASE WHEN sg.split_part IS NOT NULL THEN sg.split_part::text ELSE NULL END AS p_bundle_split_part,
      (rl.parts[2]::date + rl.day_index) AS p_log_day_date
    FROM raw_logs rl
    LEFT JOIN LATERAL (
  ...
  -- Goal lookup helper: try current-week bundle_goal first, then fall back to any
  -- active week containing the (project, label, split_part) bundle. NULL if nowhere.
  log_goal AS (
    SELECT
      lr.bundle_id,
      COALESCE(
        -- Current week (existing behaviour, but using exact match on bundle_label only)
        (SELECT bg.goal_pct
           FROM bundle_goal bg
          WHERE bg.project_id = lr.p_project_id
            AND bg.bundle_label = lr.p_bundle_label
          LIMIT 1),
        -- Fallback: any active plan row for same project+label+split → recompute proportional goal
        (
          WITH chain AS (
            SELECT
  ...
  SELECT
    ...
    CASE
      WHEN lr.p_bundle_label IS NOT NULL THEN
        CASE
          WHEN lr.p_bundle_split_part IS NOT NULL
            THEN lr.p_bundle_label || '-' || lr.p_bundle_split_part
          ELSE lr.p_bundle_label
        END
      ELSE NULL
    END AS bundle_display_label,
