...
AS $function$
  ...
  bundles_in_week AS (
    -- One row per (project, stage, bundle_label, split_part) scheduled this week
    SELECT
      ps.project_id,
      ps.stage_id,
      ps.scheduled_week,
  ...
  -- Per-bundle key (without split_part) for goal aggregation
  bundle_keys AS (
    SELECT DISTINCT
      ps.project_id,
      ps.stage_id,
  ...
  -- Total hours of the chain: split-chain across all weeks, otherwise this week only
    ...
    SELECT
      ...
      COALESCE((
        ...
        WHERE
          ...
          AND (
            (bk.split_group_id IS NOT NULL AND ps.split_group_id = bk.split_group_id)
            -- non-split bundles have no prior weeks by definition
          )
  ...
  bundle_goal AS (
        ...
        ROUND(
          CASE
            WHEN bch.chain_total_hours > 0 THEN
              (bch.chain_prior_hours + bch.this_week_hours * (ti.day_idx + 1)::numeric / 5)
    ...
    FROM bundle_chain_hours bch, today_info ti
  ),
  todays_logs AS (
    SELECT
      pdl.bundle_id,
