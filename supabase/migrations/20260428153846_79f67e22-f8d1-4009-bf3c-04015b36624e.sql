...
AS $function$
    ...
    WHERE ps.status IN ('scheduled', 'in_progress', 'paused')
      ...
      AND NOT EXISTS (
        ...
        WHERE pdl.bundle_id LIKE ps.project_id || '::%'
          AND (pdl.percent >= 100 OR pdl.phase = 'Hotovo')
          AND pdl.logged_at > (ti.rdate - interval '14 days')
      )
  ...
  bundles_in_week AS (
    ...
    SELECT
      ...
      sb.split_total, sb.split_group_id::text, sb.scheduled_hours, true AS is_spillover
      ...
      LEAST(
        ROUND(
          CASE WHEN bch.chain_total_hours > 0 THEN
            CASE
              WHEN bch.split_group_id IS NOT NULL THEN
                -- Split chain: chain end (BEZ dayFraction) = matches UI getChainWindow
                (bch.chain_prior_hours + bch.this_week_hours) / bch.chain_total_hours * 100
              ELSE
                -- Non-split bundle: prior + this_week * dayFraction
        ...
        (
          WITH chain AS (
            SELECT
              ...
              bool_or(ps.split_group_id IS NOT NULL) AS has_split
          ...
          SELECT LEAST(
            ROUND(
              CASE WHEN COALESCE(c.total, 0) > 0
                THEN
                  CASE WHEN c.has_split
                    THEN (COALESCE(c.prior, 0) + COALESCE(c.this_week, 0)) / c.total * 100
                    ELSE (COALESCE(c.prior, 0) + COALESCE(c.this_week, 0) * dp.completed_workdays / dp.week_working_days) / c.total * 100
                  END
                ELSE 0 END
            )::integer, 100
          )
  ...
  SELECT
    ...
    to_char(b.scheduled_week, 'YYYY-MM-DD')
      ...
      || CASE WHEN b.is_spillover THEN '::SPILL' ELSE '' END AS bundle_id,
    ...
    CASE WHEN b.is_spillover THEN 100 ELSE COALESCE(bg.goal_pct, 0) END AS weekly_goal_pct,
