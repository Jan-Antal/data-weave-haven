...
AS $function$
  ...
  -- Active projects this week: have logged hours (ALVENO) or daily logs.
  -- Used to filter out scheduled bundles where nobody worked yet (matches Reporta Dílna UI).
    ...
    WHERE ps.status IN ('scheduled', 'in_progress', 'paused')
      ...
      AND NOT EXISTS (
        ...
        WHERE pdl.bundle_id LIKE ps.project_id || '::%'
          AND split_part(pdl.bundle_id, '::', 4) = ps.bundle_label
  ...
  bundles_in_week AS (
    -- Scheduled bundles for current week — restricted to projects with this-week activity
    SELECT
    ...
    UNION ALL
    -- Spillover bundles always shown
    SELECT
    ...
    UNION ALL
    -- Unplanned bundles (logs without a schedule) always shown
    SELECT
  ...
  -- Today's logs (exact match on report_date)
  raw_logs_today AS (
    SELECT
  ...
  -- Last-known logs in this week for bundles that have NO log today (so daily report shows latest status)
  raw_logs_fallback AS (
    ...
    WHERE pdl.bundle_id LIKE '%::%'
      ...
      AND (split_part(pdl.bundle_id, '::', 2)::date + pdl.day_index) < report_date
      AND NOT EXISTS (
        -- Skip if same bundle has a log today
