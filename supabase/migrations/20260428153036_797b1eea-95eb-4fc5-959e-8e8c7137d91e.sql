...
AS $function$
  ...
  -- Spillover: aktívne bundle z minulých týždňov bez náprotivku v aktuálnom týždni
  spillover_bundles AS (
    ...
    WHERE ps.status IN ('scheduled', 'in_progress', 'paused')
      ...
      -- vylúč ak existuje log "Hotovo" alebo 100% v posledných 14 dňoch
      AND NOT EXISTS (
  ...
  bundles_in_week AS (
    ...
    SELECT
      ...
      sb.split_part,
      sb.split_total,
      sb.split_group_id::text AS split_group_id_text,
      sb.scheduled_hours,
